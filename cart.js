...
function notifyCartChanged() {
  ...
  queueMicrotask(() => {
    cartChangeQueued = false;
    document.dispatchEvent(new CustomEvent('dsb:cartchange'));
  });
}
...
const CartStore = function () {
  ...
  function sanitize(value) {
    ...
    Object.entries(value).forEach(([key, line]) => {
      ...
      if (!p || typeof p.id !== 'string' || key !== p.id || !Number.isInteger(line.qty) || line.qty < 1 || line.qty > 999 || !Number.isFinite(p.price) || p.price <= 0) {
        repaired = true;
        return;
      }
      valid[key] = line;
  ...
  function readAll() {
    ...
    try {
      ...
      repaired = true;
      try {
        localStorage.setItem(CART_STORAGE_KEY, JSON.stringify(memoryFallback));
      } catch (_) {}
  ...
  function appliedOrders() {
    ...
    try {
      const value = JSON.parse(localStorage.getItem(CART_STORAGE_KEY) || '{}').__dsbApplied;
  ...
  function writeAll(cart, applied = appliedOrders()) {
    ...
    if (usingFallback) {
      memoryFallback = cart;
      consumedFallback = applied;
      return;
    }
    try {
      localStorage.setItem(CART_STORAGE_KEY, JSON.stringify({
        ...cart,
        __dsbApplied: applied.slice(-100)
      }));
    } catch (e) {
      usingFallback = true;
      memoryFallback = cart;
  ...
  function locked(fn) {
    return typeof navigator !== 'undefined' && navigator.locks?.request ? navigator.locks.request('dsb-cart-write', fn) : fn();
  }
  return {
    takeRepairNotice() {
    ...
    addSafe(product, delta) {
      return locked(() => this.add(product, delta));
    },
    tryAddSafe(product, delta) {
      return locked(() => {
        const before = this.qtyFor(product.id);
        return this.add(product, delta) > before;
      });
    },
    clearSafe() {
      return locked(() => this.clear());
    },
    syncSafe(catalog) {
      return locked(() => this.syncWithCatalog(catalog));
    },
    consumeSafe(items, orderId) {
      return locked(() => {
        const applied = appliedOrders();
        if (orderId && applied.includes(orderId)) return;
        const cart = readAll();
        items.forEach(item => {
          const key = sizeCartKey(item.id, item.size),
            entry = cart[key];
          if (!entry) return;
          entry.qty -= Math.min(entry.qty, item.qty);
          if (!entry.qty) delete cart[key];
        });
    ...
    qtyFor(id) {
      return readAll()[id]?.qty || 0;
    },
    qtyForProduct(id) {
      return Object.values(readAll()).filter(x => (x.product.productId || x.product.id) === id).reduce((sum, x) => sum + x.qty, 0);
    },
    add(product, delta) {
      const cart = readAll();
      ...
      if (nextQty <= 0) delete cart[product.id];else cart[product.id] = {
        product,
        qty: nextQty
      };
      writeAll(cart);
    ...
    count() {
      return Object.values(readAll()).reduce((s, c) => s + c.qty, 0);
    },
    total() {
      return Math.round(Object.values(readAll()).reduce((s, c) => s + Math.round(c.qty * c.product.price * 100), 0)) / 100;
    },
    ...
    syncWithCatalog(catalog) {
      ...
      Object.keys(cart).forEach(id => {
        const old = cart[id].product,
          baseId = old.productId || old.id;
        ...
        if (!fresh || !Number.isFinite(fresh.price) || fresh.price <= 0 || fresh.stock === 'out of stock' || ((fresh.sizes || []).length ? !fresh.sizes.includes(old.size) : !!old.size)) {
          removed.push(old.name + (old.size ? ' (' + old.size + ')' : ''));
          delete cart[id];
          changed = true;
          return;
        }
        ...
        if (oldQty !== cart[id].qty || oldPrice !== cart[id].product.price) adjusted.push({
          name: old.name,
          size: old.size || '',
          oldQty,
          qty: cart[id].qty,
