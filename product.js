...
async function init() {
  ...
  try {
    ...
    if (root.querySelector('[data-prerendered]')) {
      ...
      if (status) {
        status.textContent = 'Live availability is temporarily unavailable. Please retry before ordering.';
        if (!root.querySelector('#pdRetryBtn')) {
          const button = document.createElement('button');
          button.id = 'pdRetryBtn';
    ...
    $('#pdRetryBtn')?.addEventListener('click', init, {
      once: true
    });
  }
}
...
function needsSizeGuide(p) {
  ...
  return cat.includes('lingerie') || cat.includes('cloth') || cat.includes('wear') || sub.includes('bra') || sub.includes('legging') || tags.includes('size-guide') || tags.includes('sizeguide') || tags.includes('sized');
}
...
function bindGallerySwipe() {
  ...
  const updateActive = () => {
    ...
    dots.forEach((d, i) => {
      ...
      d.setAttribute('aria-pressed', String(i === activeIndex));
    });
  ...
  slides.forEach((slide, i) => {
    ...
    if (img) img.addEventListener('load', () => {
      if (i === activeIndex) setGalleryHeight(i);
    });
  });
  if (slides.length > 1) {
    let raf = 0;
  ...
  if ('ResizeObserver' in window) {
    galleryResizeObserver = new ResizeObserver(() => setGalleryHeight(activeIndex));
...
function updateSeoTags(p) {
  ...
  const imageUrl = (() => {
    try {
      return new URL(p.image, CONFIG.SITE_URL + '/').href;
    } catch (_) {
      return p.image;
    }
  })();
...
function updateStructuredData(p, reviews) {
  ...
  const images = (p.gallery && p.gallery.length ? p.gallery : [p.image]).map(src => {
    try {
      return new URL(src, CONFIG.SITE_URL + '/').href;
    } catch (_) {
      return src;
    }
  });
...
function renderPdActions(p) {
  const pricing = DSB_SEO.pricing(p),
    hasSelection = pricing.sizes.includes(selectedSize);
  const price = hasSelection ? pricing.priceFor(selectedSize) : pricing.min;
  const prices = $('#pdRoot .pd-prices'),
    priceHtml = `<span class="price">${!hasSelection && pricing.min !== pricing.max ? 'From ' : ''}${money(price)}</span>${p.mrp > price ? `<span class="mrp">${money(p.mrp)}</span>` : ''}`;
  if (prices && prices.dataset.html !== priceHtml) {
    prices.innerHTML = priceHtml;
    prices.dataset.html = priceHtml;
  }
  const badge = $('#pdRoot .pd-gallery .discount'),
    discount = discountPct(p, price);
  if (badge) {
    badge.textContent = discount + '% OFF';
    badge.hidden = !discount;
  }
  if (isOutOfStock(p)) {
    delete $('#pdActions').dataset.state;
  ...
  if (p.sizes?.length && !p.sizes.includes(selectedSize)) {
    delete $('#pdActions').dataset.state;
    $('#pdActions').innerHTML = '<button class="ghost-btn" disabled style="flex:1;">Choose a size first</button>';
    return;
  }
  ...
  actions._product = p;
  actions.innerHTML = qty > 0 ? `<div class="stepper" id="pdStepper" style="height:44px;"><button type="button" data-act="dec" aria-label="Decrease quantity">−</button><span>${qty}</span><button type="button" data-act="inc" aria-label="Increase quantity" ${maxReached ? 'disabled' : ''}>+</button></div>
       <button class="primary-btn" id="pdGoCart" style="flex:1;">View cart</button>` : `<button class="ghost-btn" id="pdAdd" ${maxReached ? 'disabled' : ''} style="flex:1;">Add to cart</button>
  ...
  if (stepper) {
    ...
    if (incBtn && !incBtn.disabled) incBtn.addEventListener('click', async () => {
      await CartStore.addSafe(variant, 1);
      updateCartBadge();
      renderPdActions(p);
    });
  ...
  if (addBtn) addBtn.addEventListener('click', async () => {
    if (!(await CartStore.tryAddSafe(variant, 1))) {
...
function renderStarInput() {
  const box = $('#starInput');
  const draw = () => {
    box.innerHTML = [1, 2, 3, 4, 5].map(v => `<button type="button" class="star-btn ${v <= selectedRating ? 'filled' : ''}" data-v="${v}" aria-label="${v} stars" aria-pressed="${v === selectedRating}">★</button>`).join('');
...
document.addEventListener('dsb:languagechange', () => {
  ...
  document.querySelectorAll('.card[data-id]').forEach(card => {
    ...
    const sub = card.querySelector('.name-hindi');
    if (sub) sub.textContent = customerProductSecondaryName(product);
  });
});
document.addEventListener('dsb:catalogchange', () => {
  ...
  if (!updated) {
    ...
    renderPdActions(CURRENT_PRODUCT);
    return;
  }
  CURRENT_PRODUCT = updated;
  updateSizeOptions(updated);
  ...
  if (stock) {
    ...
    stock.className = 'pd-stock ' + (isOutOfStock(updated) ? 'out' : 'in');
  }
  updateSeoTags(updated);
  updateStructuredData(updated, CURRENT_REVIEWS);
});
document.addEventListener('DOMContentLoaded', () => {
  $('#searchTrigger').addEventListener('click', async () => {
    location.href = 'index.html?search=1';
  });
...
function reviewClientId() {
  try {
    let id = localStorage.getItem('dsb_review_client');
    if (!id) {
      id = newCheckoutId();
      localStorage.setItem('dsb_review_client', id);
    }
    return id;
  } catch (_) {
...
function updateSizeOptions(p) {
  ...
  field.querySelectorAll('.size-option').forEach(btn => btn.addEventListener('click', () => {
    ...
    renderPdActions(CURRENT_PRODUCT || p);
  }));
}
