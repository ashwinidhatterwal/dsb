...
function editorState() {
  return editorFields().map(el => ({
    id: el.id,
    value: el.type === 'checkbox' ? el.checked : el.value
  }));
}
function editorFingerprint() {
  return JSON.stringify([editorState(), currentExtraImages]);
}
function editorDirty() {
  return cleanEditor !== '' && editorFingerprint() !== cleanEditor;
}
function draftKey() {
  return 'dsb_admin_draft_v7:' + API_URL + ':' + (ADMIN_PROFILE?.name || 'Owner');
}
...
function offerSavedDraft() {
  ...
  $('#restoreDraft').onclick = () => {
    ...
    draft.fields.forEach(x => {
      const el = document.getElementById(x.id);
      if (!el || !el.closest('#tab-add')) return;
      if (el.type === 'checkbox') el.checked = !!x.value;else el.value = x.value;
    });
    currentExtraImages = Array.isArray(draft.images) ? draft.images : [];
    editingProductId = draft.id;
    editingSnapshot = draft.snapshot;
    $('#f-id').readOnly = !!editingProductId;
    $('#idLockHint').hidden = !editingProductId;
    $('#duplicateBtn').hidden = !editingProductId;
    $('#sizeOptionsField').hidden = !$('#f-hasSizes').checked;
    $('#addTabTitle').textContent = editingProductId ? 'Edit restored draft' : 'Add a product';
...
withEditorLock = async function (task) {
  ...
  try {
    return await originalEditorLock(task);
  } finally {
    persistDraft();
  }
};
...
async function adminWrite(action, payload) {
  ...
  if (data.error || data.success === false) throw new Error(data.error || 'Change was not saved.');
  return data;
}
async function archiveProduct(product) {
  ...
  try {
    await adminWrite('archiveProduct', {
      id: product.id,
      archived: archive,
...
function renderPaymentControls() {
  $$('.orow').forEach(row => {
    ...
    $('input', box).value = order.paymentreference || '';
    $('button', box).onclick = async () => {
      const controls = $$('button,select,input', box);
      controls.forEach(el => el.disabled = true);
      try {
        await adminWrite('verifyPayment', {
          orderId: order.orderid,
          paymentStatus: $('select', box).value,
          reference: $('input', box).value,
          expectedVerifiedAt: order.paymentverifiedat || ''
        });
        showToast('Payment verification recorded');
        await loadOrders();
      } catch (err) {
        showToast(err.message);
      } finally {
...
async function loadArchive() {
  ...
  try {
    const data = await adminRead('adminProductsPage', {
      archived: true,
    ...
    $('#archiveList').innerHTML = data.products.length ? data.products.map(p => `<div class="archive-card"><img src="${escapeHtml(p.image || '')}" alt="" loading="lazy"><div><strong>${escapeHtml(p.name)}</strong><small>${escapeHtml(p.id)} · ${escapeHtml(p.category)} · ₹${Number(p.price) || 0}</small></div><div class="archive-actions"><button class="ghost-btn archive-restore" data-restore="${escapeHtml(p.id)}" ${ADMIN_PROFILE?.role === 'viewer' ? 'disabled' : ''} aria-label="Restore ${escapeHtml(p.name)}" >Restore</button>${ADMIN_PROFILE?.role === 'admin' ? `<button class="ghost-btn archive-delete" data-delete="${escapeHtml(p.id)}" aria-label="Delete ${escapeHtml(p.name)} permanently" >Delete</button>` : ''}</div></div>`).join('') : '<p class="hint">No archived products.</p>';
    ...
    if (sequence === archiveSequence) $('#archiveStatus').textContent = err.message;
  }
}
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  $('#tab-add').addEventListener('input', () => {
  ...
  new MutationObserver(() => {
    clearTimeout(draftTimer);
    draftTimer = setTimeout(persistDraft, 300);
  }).observe($('#extraImagesPreview'), {
    childList: true
  });
  window.addEventListener('beforeunload', e => {
    if (editorDirty() || editorBusy) {
      persistDraft();
      e.preventDefault();
      e.returnValue = '';
    }
  ...
  $('#archiveList').onclick = e => {
    ...
    const id = button.dataset.restore || button.dataset.delete;
    const product = archivedProducts.find(p => String(p.id) === id);
    if (!product) return;
    if (button.dataset.restore) archiveProduct(product);else permanentlyDeleteProduct(product);
  };
  $('#signOutBtn').onclick = () => {
    if (editorBusy) return showToast('Wait for the current operation to finish.');
    if (editorDirty() && !confirm('Sign out? Your draft will remain in this tab for this staff key.')) return;
    persistDraft();
    cleanEditor = editorFingerprint();
    ADMIN_KEY = '';
    $('#adminKey').value = '';
