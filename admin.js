...
async function withEditorLock(task) {
  if (editorBusy) {
    showToast('Please wait for the current save or upload.');
    return;
  }
  editorBusy = true;
  try {
...
async function loadProducts(refreshRelated = true) {
  ...
  try {
    if (!ADMIN_PROFILE) {
      ...
      if (profile.version !== 22) throw new Error('Deploy the new code.gs version before opening this admin update.');
    ...
    productFilter = $('#filterInput').value.trim().toLowerCase();
...
async function loadOrders() {
  const wrap = $('#orderList'),
    sequence = ++orderRequestSequence;
  $$('button,select', wrap).forEach(el => el.disabled = true);
  try {
    const options = {
      query: $('#orderFilterInput').value.trim(),
    ...
    if (sequence === orderRequestSequence) {
      wrap.innerHTML = `<p class="hint">Could not load orders: ${escapeHtml(err.message)}</p><button type="button" class="ghost-btn" id="retryOrders">Retry</button>`;
      $('#retryOrders').onclick = loadOrders;
    }
    return false;
  }
}
function orderPriceBreakdownHtml(order) {
  const total = Number(order.total) || 0;
  const discount = Number(order.discount) || 0;
  const delivery = Number(order.deliverycharge) || 0;
  const cod = Number(order.codcharge) || 0;
  const subtotal = Math.round((total + discount - delivery - cod) * 100) / 100;
  const amount = value => '₹' + value.toLocaleString('en-IN', {minimumFractionDigits: 2, maximumFractionDigits: 2});
  return `<dl class="order-price-breakdown" aria-label="Order price breakdown">
    <div><dt>Subtotal</dt><dd>${amount(subtotal)}</dd></div>
    <div><dt>Discount${order.promocode ? ' (' + escapeHtml(order.promocode) + ')' : ''}</dt><dd>−${amount(discount)}</dd></div>
    <div><dt>Delivery</dt><dd>${amount(delivery)}</dd></div>
    <div><dt>COD fee</dt><dd>${amount(cod)}</dd></div>
    <div class="order-price-total"><dt>Total</dt><dd>${amount(total)}</dd></div>
  </dl>`;
}
...
async function updateOrderStatus(orderId, newStatus) {
  ...
  try {
    ...
    pendingOrderUpdates.delete(orderId);
...
function renderProductList() {
  ...
  $$('.product-pagination').forEach(pager => {
    ...
    const pages = [...new Set([0, ...Array.from({
      length: Math.min(51, pageCount)
    }, (_, i) => start + i), pageCount - 1])].sort((a, b) => a - b);
    ...
    const rail = $('.pager-numbers', pager),
      current = $('[aria-current="page"]', rail);
    ...
    pager.onclick = async e => {
      ...
      if (next === productPage) return;
      productPage = Math.max(0, Math.min(pageCount - 1, next));
      pager.querySelectorAll('button').forEach(el => el.disabled = true);
      if (productResponse) {
        if (!(await loadProducts(false))) {
          renderProductList();
          return;
        }
  ...
  wrap.onclick = e => {
    const button = e.target.closest('button[data-act]'),
      row = button?.closest('.arow');
    if (!row || button.disabled) return;
    const product = byId.get(row.dataset.id);
    if (!product) return;
    if (button.dataset.act === 'edit') fillForm(product);else archiveProduct(product);
  };
}
function fillForm(p) {
  if (editorBusy) {
...
function clearForm() {
  ...
  $('#idLockHint').hidden = true;
  $('#duplicateBtn').hidden = true;
  ['f-id', 'f-name', 'f-nameHindi', 'f-category', 'f-subcategory', 'f-price', 'f-mrp', 'f-costprice', 'f-image', 'f-description', 'f-stockqty', 'f-tags'].forEach(id => $('#' + id).value = '');
...
async function uploadImage() {
  return withEditorLock(uploadImageTask);
}
...
async function uploadExtraImage() {
  return withEditorLock(uploadExtraImageTask);
}
...
async function saveProduct() {
  return withEditorLock(saveProductTask);
}
async function saveProductTask() {
  ...
  if (!Number.isFinite(product.price) || product.price <= 0) {
    status(statusEl, 'Price must be a positive number.', false);
    return;
  }
  for (const key of ['mrp', 'costprice', 'stockqty']) {
    const value = product[key];
    if (value !== '' && (!Number.isFinite(value) || value < 0 || key === 'stockqty' && !Number.isInteger(value))) {
      status(statusEl, `Enter a valid non-negative ${key === 'stockqty' ? 'whole stock quantity' : key === 'mrp' ? 'MRP' : 'cost price'}.`, false);
      return;
    }
  }
  ...
  if (isUpdate) {
    ...
    product.expected_stockqty = original?.stockqty ?? '';
    product.expected_stock = original?.stock ?? '';
  }
  ...
  if (!isUpdate) {
    ...
    try {
      saveRequest = JSON.parse(sessionStorage.getItem('dsb_admin_add_attempt') || 'null');
      if (!saveRequest || saveRequest.fingerprint !== fingerprint) {
        saveRequest = {
          requestId: crypto.randomUUID(),
          fingerprint
        };
        sessionStorage.setItem('dsb_admin_add_attempt', JSON.stringify(saveRequest));
      }
    } catch (_) {
      status(statusEl, 'Enable browser storage before adding products safely.', false);
      return;
    }
  }
  ...
  try {
    const res = await adminFetch(API_URL, {
      ...
      // avoids CORS preflight against Apps Script
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  workspaceSearch.addEventListener('keydown', e => {
    if (e.key === 'Enter') {
      switchTab('products');
      $('#filterInput').value = workspaceSearch.value;
      loadProducts(false);
      $('#filterInput').focus();
    }
  });
  document.addEventListener('keydown', e => {
    if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'k' && $('#adminApp').style.display === 'block') {
      e.preventDefault();
      switchTab('products');
      $('#filterInput').focus();
    }
  ...
  $('#orderFilterInput').addEventListener('input', () => {
    clearTimeout(orderSearchTimer);
    orderSearchTimer = setTimeout(reloadOrders, 250);
  });
...
$('#f-hasSizes').addEventListener('change', () => {
  $('#sizeOptionsField').hidden = !$('#f-hasSizes').checked;
});
