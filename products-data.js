...
function normalizeRows(rows) {
  return (rows || []).filter(r => r && (r.id || r.ID)).map(r => {
    ...
    const extraImages = String(r.images ?? r.Images ?? '').split(',').map(s => s.trim()).filter(Boolean);
    return {
      id: String(r.id ?? r.ID ?? '').trim(),
      searchText: [r.id, r.name, r.Name, r.namehindi, r.nameHindi, r.category, r.subcategory, r.tags].filter(Boolean).join(' ').toLowerCase(),
...
function applyCatalogRows(rows, source, time) {
  const products = normalizeRows(rows),
    changed = JSON.stringify(products) !== JSON.stringify(ALL_PRODUCTS);
...
async function refreshLiveCatalog() {
  ...
  try {
    return await catalogLiveRequest;
  } finally {
    catalogLiveRequest = null;
  }
}
async function loadAllProducts(options = {}) {
  ...
  catalogRequest = (async () => {
    ...
    const snapshot = requestJson('catalog-snapshot.json', {
      cache: 'no-cache'
    }, 5000).then(data => {
...
async function cachedPublicJson(key, url, ttl = 180000, force = false) {
  const storageKey = 'dsb_public_' + key,
    api = CONFIG.SHEET_API_URL;
  if (!force) {
    try {
      const c = JSON.parse(sessionStorage.getItem(storageKey) || 'null');
      if (c?.api === api && Date.now() - c.time < ttl) return c.data;
    } catch (_) {}
  }
...
function invalidateReviewCache(productId) {
  for (const key of ['review-summary', 'reviews-' + productId]) {
    try {
      sessionStorage.removeItem('dsb_public_' + key);
    } catch (_) {}
  }
}
...
async function loadReviewSummaries() {
  ...
  try {
    ...
    Object.keys(totals).forEach(pid => {
      summary[pid] = {
        avg: totals[pid].sum / totals[pid].count,
