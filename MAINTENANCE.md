...
## Analytics v3 reliability notes (2026-09-22)
- `storefront-analytics.js` owns anonymous browser tracking. Sessions are shared across tabs and rotate after 30 minutes of inactivity. Every queued event carries `visitorId`, `sessionId`, and a stable `qid`.
- `src/backend/analytics.gs` is the source of truth for analytics ingestion/reporting. `code.gs` is generated by `node scripts/build.mjs`; edit the source module, not the generated analytics copy.
- Analytics retries are idempotent for v3 events: `qid` is persisted in the hidden Analytics sheet and protected by Script Cache plus a recent-sheet tail check.
- Order/revenue KPIs remain sourced from Orders. New orders also store hashed anonymous analytics attribution columns so a successful order can still be attributed when the browser's final analytics event is lost.
- Search analytics records only a paused, completed query result count. Raw search text is intentionally not stored.
