...
function page(title,description,url,body,site,lang='en',schema={},alternate=''){
 return `<!DOCTYPE html><html lang="${lang}"><head><!-- Google tag (gtag.js) --><script async src="https://www.googletagmanager.com/gtag/js?id=G-CMELC4RJMD"></script><script>window.dataLayer = window.dataLayer || []; function gtag(){dataLayer.push(arguments);} gtag('js', new Date()); gtag('config', 'G-CMELC4RJMD');</script><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><base href="${esc(site)}/"><title>${esc(title)}</title><meta name="description" content="${esc(description)}"><meta name="robots" content="index,follow,max-image-preview:large"><link rel="canonical" href="${esc(url)}">${alternate}<meta property="og:type" content="website"><meta property="og:site_name" content="Dhatterwal Suhag Bhandar"><meta property="og:title" content="${esc(title)}"><meta property="og:description" content="${esc(description)}"><meta property="og:url" content="${esc(url)}"><meta property="og:image" content="${esc(site)}/social-card.png"><meta name="twitter:card" content="summary_large_image"><meta name="twitter:title" content="${esc(title)}"><meta name="twitter:description" content="${esc(description)}"><meta name="twitter:image" content="${esc(site)}/social-card.png"><link rel="icon" href="favicon-48.png"><link rel="stylesheet" href="style.css?v=20260914checkout"><script type="application/ld+json">${json(schema)}</script></head><body><main class="seo-category"><nav><a href="${lang==='hi'?'hi/':'index.html'}">${lang==='hi'?'दुकान':'Shop'}</a> · <a href="catalog.html">${lang==='hi'?'सभी उत्पाद':'All products'}</a> · <a href="contact.html">${lang==='hi'?'संपर्क':'Contact'}</a></nav>${body}</main></body></html>`;
}
export async function generateStaticProducts(destination,rows,template,site){
 ...
 const catalogFile=path.join(destination,'catalog.html');let catalog=await fs.readFile(catalogFile,'utf8');catalog=catalog.replace(/(<div[^>]*id="catalogRoot"[^>]*>)[\s\S]*?(<\/div>)/,(_,x,y)=>x+`<nav class="seo-category-links">${categoryLinks}</nav><div class="seo-products">${products.map(p=>item(p,site)).join('')}</div>`+y);
 const catalogList={'@context':'https://schema.org','@type':'ItemList',name:'Dhatterwal Suhag Bhandar product catalogue',numberOfItems:products.length,itemListElement:products.map((p,i)=>({'@type':'ListItem',position:i+1,url:site+'/'+productPath(p.id),item:{'@type':'Product',name:seo.name(p),sku:String(p.id),url:site+'/'+productPath(p.id),...(p.image&&/^https:\/\//.test(p.image)?{image:optimizeImage(p.image,1200)}:{})}}))};
 catalog=catalog.replace('</head>',`<script type="application/ld+json" id="catalogItemListLd">${json(catalogList)}</script></head>`);await fs.writeFile(catalogFile,catalog);
 ...
 await fs.writeFile(path.join(destination,'hi/index.html'),page('धत्तरवाल सुहाग भंडार | गोलूवाला','गोलूवाला, राजस्थान की धत्तरवाल सुहाग भंडार दुकान के उत्पाद देखें। ऑनलाइन ऑर्डर और दुकान से संपर्क की सुविधा।',site+'/hi/',hiBody,site,'hi',{'@context':'https://schema.org','@type':'WebPage','@id':site+'/hi/#webpage',url:site+'/hi/',inLanguage:'hi-IN',name:'धत्तरवाल सुहाग भंडार',isPartOf:{'@id':site+'/#website'},about:{'@id':site+'/#store'},publisher:{'@id':site+'/#store'}},alternates(site+'/',site+'/hi/')));
 ...
 const merchant=`<?xml version="1.0" encoding="UTF-8"?>\n<rss xmlns:g="http://base.google.com/ns/1.0" version="2.0"><channel><title>Dhatterwal Suhag Bhandar</title><link>${esc(site)}/</link><description>Official product catalogue of Dhatterwal Suhag Bhandar (DSB), Goluwala, Rajasthan.</description>${merchantItems}</channel></rss>\n`;
