...
async function loadProductsForShop() {
  ...
  try {
    ...
    $('#catalogRetry').addEventListener('click', loadProductsForShop, {
      once: true
    });
    return;
  }
...
function carouselProducts() {
  return ALL_PRODUCTS.filter(p => !isOutOfStock(p) && (activeCategory === 'All' || p.category === activeCategory) && (activeSubcategory === 'All' || p.subcategory === activeSubcategory));
}
function popularProducts(limit) {
  const available = carouselProducts();
  const score = p => {
    const r = reviewSummaryFor(p.id);
    return r?.count ? r.avg * Math.log1p(r.count) : 0;
  };
...
function renderCategoryRail() {
  ...
  rail.innerHTML = cats.map(c => `<button class="chip ${c === activeCategory ? 'active' : ''}" data-cat="${escapeHtml(c)}">${escapeHtml(c)}</button>`).join('');
...
function createPager(container, sentinelEl) {
  ...
  function reset(newList, emptyMessage, preserve = false) {
    ...
    do {
      renderNextBatch();
    } while (renderedCount < Math.min(target, list.length));
  }
...
function sortProducts(list) {
  ...
  if (sortMode === 'price-asc' || sortMode === 'price-desc') return list.map(p => ({
    p,
    price: DSB_SEO.pricing(p).min
  })).sort((a, b) => sortMode === 'price-asc' ? a.price - b.price : b.price - a.price).map(x => x.p);
  return list;
}
...
function initUI() {
  ...
  if (stickyBar) {
    ...
    syncCategoryOffset();
    if ('ResizeObserver' in window) new ResizeObserver(syncCategoryOffset).observe(stickyBar);else window.addEventListener('resize', syncCategoryOffset, {
      passive: true
    });
  }
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    section.setAttribute('aria-label', 'Available offers');
    ...
    section.addEventListener('click', async e => {
      ...
      if (!b) return;
      try {
        await navigator.clipboard.writeText(b.dataset.offer);
