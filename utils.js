...
function openDialogFocus(root, onClose) {
  ...
  for (let node = root; node && node !== document.body; node = node.parentElement) {
    ...
    Array.from(node.parentElement.children).forEach(sibling => {
      ...
      entry.siblings.push([sibling, sibling.inert]);
      sibling.inert = true;
    });
  }
...
document.addEventListener('keydown', e => {
  ...
  if (!first) {
    e.preventDefault();
    entry.root.focus();
  } else if (e.shiftKey && (document.activeElement === first || !entry.root.contains(document.activeElement))) {
    e.preventDefault();
    last.focus();
  } else if (!e.shiftKey && (document.activeElement === last || !entry.root.contains(document.activeElement))) {
    e.preventDefault();
    first.focus();
  }
}, true);
