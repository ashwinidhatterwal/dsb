...
(function () {
  ...
  function render(products) {
    ...
    root.innerHTML = Array.from(groups.entries()).map(([cat, items]) => `
      ...
      return `<a class="catalog-item" href="${productHref(p)}">
            <strong>${escapeHtml(p.name)}</strong>
            <span>${escapeHtml(p.subcategory || '')} · ${prices.min !== prices.max ? 'From ' : ''}${money(prices.min)} · ${isOutOfStock(p) ? 'Out of stock' : 'In stock'}</span>
          </a>`;
    }).join('')}
        </div>
      </section>`).join('');
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    try {
      render(await loadAllProducts());
    } catch (err) {
      console.error('Catalogue load failed', err);
      const root = document.getElementById('catalogRoot');
      if (root.querySelector('a.catalog-item')) root.insertAdjacentHTML('afterbegin', '<p class="catalog-status" role="status">Showing saved products. Current availability is checked before ordering.</p>');else root.innerHTML = 'Could not load the catalogue right now. <a href="index.html">Return to the shop</a>';
    }
  });
})();
