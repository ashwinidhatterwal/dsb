(function () {
  ...
  function scheduleBanner() {
    ...
    bannerTimer = setTimeout(function () {
      bannerTimer = null;
      if (document.hidden || document.querySelector('.overlay.open,.search-overlay.open')) {
        scheduleBanner();
        return;
      }
  ...
  function ensureBanner(mode) {
    ...
    banner.innerHTML = '<div class="install-banner-mark" aria-hidden="true"><img src="icon-192.png" alt=""></div>' + '<div class="install-banner-copy"><strong>Add Suhag Bhandar to Home Screen</strong><span>' + (nativeReady ? 'Install the shop for quicker access.' : manualIOS ? 'Keep the shop one tap away on your iPhone or iPad.' : 'Keep the shop one tap away on your phone.') + '</span></div>' + '<div class="install-banner-actions">' + '<button type="button" class="install-btn" data-install>' + (nativeReady ? 'Install' : 'Add') + '</button>' + '<button type="button" class="install-dismiss" data-dismiss aria-label="Dismiss install suggestion">×</button></div>';
